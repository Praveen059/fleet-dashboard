...
# Sidebar (fixed, neutral style)
st.sidebar.title("Fleet Management")
nav = st.sidebar.radio(
    "Section",
    ["Fleet Overview", "Vehicles", "Drivers", "Fuel Savings", "Fleet Comparison", "Maintenance/CPKM", "CO2/Environmental", "Compliance", "Cost Analysis"],
)

# Always show KPIs at top
st.markdown("""
<div style='padding:12px 0 1px 0;margin-bottom:0;'>
    <h1 style='margin-bottom:10px;font-size:2.1rem;color:#246eff;'>Fleet Management Dashboard</h1>
</div>
...
with col4:
    st.metric("Monthly Maintenance CPKM", f"₹{df_vehicles['Maintenance CPKM (₹)'].mean():.2f}")
...
# === Vehicles ===
    ...
    search = st.text_input("Search Vehicle ID")
    vehicles_df = df_vehicles.copy()
    if search:
        vehicles_df = vehicles_df[vehicles_df["Vehicle ID"].str.contains(search, case=False)]
    status_filter = st.selectbox("Status Filter", ["All"] + sorted(df_vehicles["Status"].unique()))
    ...
    selected = st.selectbox("Select Vehicle ID", vehicles_df["Vehicle ID"])
    if selected:
...
# === Drivers ===
    ...
    search = st.text_input("Search Driver Name")
    ...
    selected = st.selectbox("Select Driver Name", drivers_df["Driver Name"])
    if selected:
        ...
        with col4:
            st.metric("Fuel Savings Contributed", f"₹{d['Fuel Savings Contributed (₹)']}")
